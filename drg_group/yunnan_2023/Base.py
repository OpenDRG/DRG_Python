...
MedicalRecord=namedtuple('MedicalRecord','Index,gender,age,ageDay,weight,dept,inHospitalTime,leavingType,zdList,ssList,amount')
...
def create_result_check(index,status,messages):
  return GroupResult(index,status.value,messages,'','','')

def create_result(index,messages,drgCode):
  if drgCode=='0000':
    return GroupResult(index,DrgGroupStatus.FAIL.value,messages,'0000','00','0000A')
  elif drgCode[1:3]=='00':
    return GroupResult(index,DrgGroupStatus.FAIL.value,messages,'MDC'+drgCode[0],'00','0000B')
  elif drgCode[1:3]=='QY':
    return GroupResult(index,DrgGroupStatus.QY.value,messages,'MDC'+drgCode[0],'QY',drgCode)
  else:
    return GroupResult(index,DrgGroupStatus.SUCCESS.value,messages,'MDC'+drgCode[0],drgCode[:3],drgCode)

reader=Reader('DATA')
MCC=reader.read('MCC').to_dict()
CC=reader.read('CC').to_dict()
CCE=reader.read('CCE').to_dict()
SS_VALID=reader.read('SS_VALID').to_list()
