...
from drg_group.anyang_2021.DRG import MDCD_DRG
...
def group(record):
  ...
  if True and record.ssList and record.ssList[0] in adrg_ss and record.ssList and intersect(record.ssList,adrg_ss):
    message('符合DG1入组条件，匹配规则：主手术匹配、某一手术匹配')
    
    if MDCD_DRG.DG1AA_group(record):
      return 'DG12A'

    if MDCD_DRG.DG1AB_group(record):
      return 'DG12B'

    if MDCD_DRG.DG15A_group(record):
      return 'DG15A'

    if MDCD_DRG.DG15B_group(record):
      return 'DG15B'
