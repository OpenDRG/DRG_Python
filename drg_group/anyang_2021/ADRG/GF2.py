...
def group(record):
  ...
  if True and record.ssList and record.ssList[0] in adrg_ss and record.ssList and intersect(record.ssList,adrg_ss):
    message('符合GF2入组条件，匹配规则：主手术匹配、某一手术匹配')
    
    if MDCG_DRG.GF21A_group(record):
      return 'GF21A'

    if MDCG_DRG.GF21B_group(record):
      return 'GF21B'

    if MDCG_DRG.GF23A_group(record):
      return 'GF23A'

    if MDCG_DRG.GF23B_group(record):
      return 'GF23B'

    if MDCG_DRG.GF25A_group(record):
      return 'GF25A'

    if MDCG_DRG.GF25B_group(record):
      return 'GF25B'
