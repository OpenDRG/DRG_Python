...
from drg_group.anyang_2021.DRG import MDCE_DRG
...
def group(record):
  ...
  if True and record.ssList and record.ssList[0] in adrg_ss and record.ssList and intersect(record.ssList,adrg_ss):
    message('符合EB1入组条件，匹配规则：主手术匹配、某一手术匹配')
    
    if MDCE_DRG.EB1AA_group(record):
      return 'EB12A'

    if MDCE_DRG.EB1AB_group(record):
      return 'EB12B'

    if MDCE_DRG.EB15A_group(record):
      return 'EB15A'

    if MDCE_DRG.EB15B_group(record):
      return 'EB15B'
