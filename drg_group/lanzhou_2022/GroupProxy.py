...
from drg_group.lanzhou_2022.Grouper_lanzhou_2022 import Grouper_lanzhou_2022 as Grouper
from drg_group.lanzhou_2022.Base import Reader,MedicalRecord,DrgGroupStatus,GroupResult,tuple_to_str
...
class GroupProxy:
  ...
  def check(self,record):
    try:
      ...
      if not (record.gender in [1,'1','男','1.0'] or record.gender in [2,'2','女','2.0']):
        self.message('病人性别取值必须为1或2：{}'.format(record.gender))
        return DrgGroupStatus.CHECK_FAILED
      if record.age==None:
        self.message('病人年龄为空')
        return DrgGroupStatus.CHECK_FAILED
      if int(float(record.age))==0 and record.ageDay==None:
        self.message('病人年龄0时，年龄天数必须有值')
        return DrgGroupStatus.CHECK_FAILED
      if int(float(record.age))==0 and int(float(record.ageDay))<=28 and record.weight==None:
  ...
  def group_record(self,record_str):
    record=MedicalRecord(**dict(zip(MedicalRecord._fields,replace_csv(record_str).split(','))))
